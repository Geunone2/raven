  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        // drizzle의 notInArray는 mutable 배열 타입을 요구해서, as const 튜플인
        // NON_CONTRIBUTION_CONTENT_TYPES를 그대로 넘기면 타입 에러가 난다(런타임
        // 값은 동일 — 리팩토링과 무관하게 이번에 build 검증하다 발견한 기존
        // 버그라 같이 고쳤다, 2026-08-15).
